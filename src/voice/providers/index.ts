 ...
 */
export function buildPrimaryProvider(env: ProviderEnv): VoiceProvider {
  ...
  switch (choice) {
    ...
    default:
      throw new Error(
        `Unknown VOICE_PROVIDER: ${env.VOICE_PROVIDER} — expected 'azure_voice_live' or 'deepgram'`,
      );
  }
}
 ...
 */
export function buildFallbackProvider(env: ProviderEnv): VoiceProvider | null {
...
export async function startWithFallback(
  ...
  try {
    ...
    if (!fallback) {
      logger.error(
        `voice: primary provider=${primary.name} failed; no fallback configured — voice disabled`,
        { error: msg },
      );
      throw err;
    }
    logger.warn(
      `voice: primary provider=${primary.name} failed; falling back to provider=${fallback.name}`,
      { error: msg },
    );
