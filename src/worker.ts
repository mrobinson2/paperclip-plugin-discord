...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    let token: string | null = tokenFromFile ?? tokenFromEnv ?? null;
    if (!token && config.discordBotTokenRef) {
      try {
        token = await ctx.secrets.resolve(config.discordBotTokenRef);
      } catch (err) {
        ctx.logger.warn(
          "discord plugin: ctx.secrets.resolve(discordBotTokenRef) failed; " +
          "set DISCORD_BOT_TOKEN env var to bypass the secret store",
    ...
    if (!token) {
      ctx.logger.warn(
        "No bot token available (/secrets/discord-bot-token absent AND " +
    ...
    if (!paperclipBoardApiKey && config.paperclipBoardApiKeyRef) {
      try {
        ...
        ctx.logger.warn(
          "discord plugin: ctx.secrets.resolve(paperclipBoardApiKeyRef) failed; " +
          "set PAPERCLIP_BOARD_API_KEY env var to bypass the secret store",
          { error: err instanceof Error ? err.message : String(err) },
        );
      }
    }
