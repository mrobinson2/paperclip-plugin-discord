...
export function classifyMedia(attachment) {
...
export function detectMedia(attachments) {
    const results = [];
...
export async function transcribeAudio(ctx, audioUrl, companyId) {
    try {
        ...
        return result.runId ? "Transcription started — results will appear when ready." : null;
    }
    catch (err) {
...
export async function routeToBriefAgent(ctx, companyId, content, sourceChannelId, sourceMessageId) {
    try {
...
export async function processMediaMessage(ctx, token, channelId, messageId, attachments, companyId) {
    ...
    for (const { attachment, mediaType } of detected) {
        ...
        if (mediaType === "image") {
            ...
            await routeToBriefAgent(ctx, companyId, `Image attachment: ${attachment.url} (${attachment.filename})`, channelId, messageId);
        }
