...
import { getWorkflowStore, saveWorkflowStore, runWorkflow, resumeWorkflowAfterApproval, BUILTIN_COMMANDS, } from "./workflow-engine.js";
...
export async function handleInteraction(ctx, interaction, cmdCtx) {
...
async function handleSlashCommand(ctx, data, member, cmdCtx, interactionChannelId) {
    ...
    if (data.name === "acp") {
        return handleAcpCommand(ctx, cmdCtx?.token ?? "", data, companyId, cmdCtx?.defaultChannelId ?? "");
    }
    ...
    switch (subName) {
        ...
        case "approve":
            return handleApprove(ctx, getOption(subcommand.options ?? [], "id"), member?.user.username, baseUrl, cmdCtx?.paperclipBoardApiKey);
        case "budget":
        ...
        case "digest":
            return handleDigest(ctx, getOption(subcommand.options ?? [], "action") ?? "status", getOption(subcommand.options ?? [], "mode"));
        case "commands":
...
async function handleAutocomplete(ctx, data, cmdCtx) {
    const subcommand = data.options?.[0];
    if (!subcommand)
...
async function handleStatus(ctx, companyId) {
    try {
        ...
        const agents = allAgents.filter((a) => a.status === "active" || a.status === "running");
        ...
        const doneList = doneIssues.length > 0
            ? doneIssues.map((i) => `- **${i.identifier ?? i.id}** ${i.title ?? ""}`).join("\n")
...
async function handleApprove(ctx, approvalId, username, baseUrl, apiKey) {
    if (!approvalId) {
        return respondToInteraction({
            type: 4,
...
async function handleBudget(ctx, agentQuery, companyId) {
    if (!agentQuery) {
        return respondToInteraction({
            type: 4,
    ...
    try {
        ...
        const agent = agents.find((a) => a.id === agentQuery || a.name === agentQuery ||
...
async function handleIssues(ctx, companyId, projectFilter, baseUrl) {
    try {
...
async function handleAgents(ctx, companyId, companyFilter, baseUrl) {
    try {
        let resolvedCompanyId = companyId;
        let companyLabel;
        if (companyFilter) {
            const companies = await ctx.companies.list();
            const match = companies.find((c) => c.id === companyFilter || c.name?.toLowerCase() === companyFilter.toLowerCase());
            if (!match) {
...
async function handleProjects(ctx, companyId, companyFilter) {
    try {
        let resolvedCompanyId = companyId;
        let companyLabel;
        if (companyFilter) {
...
async function handleConnect(ctx, companyArg) {
    if (!companyArg?.trim()) {
        try {
            const companies = await ctx.companies.list();
            const names = companies.map((c) => c.name || c.id).join(", ");
    ...
    try {
        ...
        const companies = await ctx.companies.list();
        const match = companies.find((c) => c.id === input || c.name?.toLowerCase() === input.toLowerCase());
        if (!match) {
            const names = companies.map((c) => c.name || c.id).join(", ");
            return respondToInteraction({
                type: 4,
        ...
        await ctx.state.set({ scopeKind: "instance", stateKey: `company_default` }, { companyId: match.id, companyName: match.name ?? input, linkedAt: new Date().toISOString() });
        return respondToInteraction({
            type: 4,
            embeds: [{
...
async function handleConnectChannel(ctx, projectName, channelId) {
    if (!projectName.trim()) {
        return respondToInteraction({
            type: 4,
    ...
    try {
        ...
        await ctx.state.set({ scopeKind: "instance", stateKey: "channel-project-map" }, channelMap);
        return respondToInteraction({
            type: 4,
            embeds: [{
...
async function handleDigest(ctx, action, mode) {
    ...
    if (action === "off") {
        await ctx.state.set({ scopeKind: "instance", stateKey }, { mode: "off", enabled: false });
        return respondToInteraction({
            type: 4,
            embeds: [{
    ...
    if (action === "on") {
        ...
        await ctx.state.set({ scopeKind: "instance", stateKey }, { mode: digestMode, enabled: true });
        return respondToInteraction({
            type: 4,
            embeds: [{
...
async function handleButtonClick(ctx, data, username, cmdCtx) {
    ...
    if (customId.startsWith("issue_assign_")) {
        ...
        try {
            const issueCompanyId = await resolveIssueCompanyId(ctx, issueId);
            const issue = await ctx.issues.get(issueId, issueCompanyId);
            ...
            await ctx.issues.update(issueId, { assigneeUserId: `discord:${actor}` }, issueCompanyId);
        }
        catch (err) {
...
async function handleEscalationButton(ctx, customId, actor, _baseUrl) {
    ...
    const uuidEscMatch = remaining.match(/^([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})_(esc_.+)$/i);
    ...
    const record = await getEscalation(ctx, escalationId, companyIdForLookup);
    if (!record) {
    ...
    const resolveRecord = async (resolution) => {
        record.status = "resolved";
        await ctx.state.set({ scopeKind: "company", scopeId: companyId, stateKey: `escalation_${escalationId}` }, {
            ...record,
...
async function resolveIssueCompanyId(ctx, issueId) {
    const companies = await ctx.companies.list();
...
// ---------------------------------------------------------------------------
async function handleCommands(ctx, subcommandGroup, cmdCtx) {
    ...
    switch (sub.name) {
        ...
        case "run":
            return handleCommandsRun(ctx, companyId, baseUrl, token, channelId, getOption(sub.options ?? [], "name") ?? "", getOption(sub.options ?? [], "args") ?? "", cmdCtx?.paperclipBoardApiKey ?? "");
        case "delete":
...
async function handleCommandsImport(ctx, companyId, jsonStr) {
    if (!jsonStr?.trim()) {
        return respondToInteraction({
            type: 4,
...
async function handleCommandsList(ctx, companyId) {
    const store = await getWorkflowStore(ctx, companyId);
...
async function handleCommandsRun(ctx, companyId, baseUrl, token, channelId, name, args, paperclipBoardApiKey) {
    if (!name.trim()) {
        return respondToInteraction({
            type: 4,
...
async function handleCommandsDelete(ctx, companyId, name) {
    if (!name.trim()) {
        return respondToInteraction({
            type: 4,
...
// ---------------------------------------------------------------------------
async function handleWorkflowApprovalButton(ctx, customId, actor, cmdCtx) {
    ...
    const result = await resumeWorkflowAfterApproval(ctx, token, channelId, companyId, baseUrl, approvalId, approved, cmdCtx?.paperclipBoardApiKey ?? "");
