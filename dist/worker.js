 ...
 */
export async function enrichRunEventPayload(ctx, event) {
    const payload = { ...event.payload };
    ...
    try {
        ...
        if (payload.issueId && (payload.issueIdentifier == null || payload.issueTitle == null)) {
            const issue = await ctx.issues.get(String(payload.issueId), companyId);
            if (issue) {
                if (payload.issueIdentifier == null)
                    payload.issueIdentifier = issue.identifier ?? null;
                if (payload.issueTitle == null)
...
const plugin = definePlugin({
    async setup(ctx) {
        ...
        const gateway = await connectGateway(ctx, token, async (interaction) => {
            ...
            ? async (message) => {
                if (!message.author?.bot)
                    ctx.metrics.write(METRIC_NAMES.msgReceived, 1).catch(() => { });
